...
| `bin/sweep-report.mjs` | Turns a failing sweep into ONE standing task, first filed as "Walks that fail in the full sweep", updated in place on every later sweep instead of filed again. It finds that task by the `standing: "walk-sweep"` mark it carries rather than by its title: until 2026-09-17 it matched on the title, the daily triage renamed the task, and the next two sweeps filed duplicates beside it while the four-walk triage sat on a task nothing was writing to. It rewrites only the block between the two markers in `bin/sweep-notes.mjs`; anything written below the end marker is a person's triage and is carried across untouched. Until 2026-09-13 it replaced the whole notes field, and a manager pass's triage of twelve failing walks lasted four hours. |
