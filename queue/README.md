...
| `bin/sweep-notes.mjs` | The two halves of those notes: the sweep's block (the list, which failures another open task already names, which walks stopped failing since last time) and the part below it that only a person writes. Drill: `node queue/bin/sweep-notes-drill.mjs`. |
...
| `bin/sweep-notes-drill.mjs` | Asserts a sweep result cannot wipe what a person wrote on the standing failing-walks task: two sweeps with a hand-written paragraph added in between keep the paragraph and update the list, notes written before the markers existed are migrated rather than lost, a walk that passes again is named as having stopped failing (and a cut-short sweep claims nothing), and each failure another open task already names says so. Run it after touching `sweep-notes.mjs` or `sweep-report.mjs`. |
