...
| `bin/sweep-parse-drill.mjs` | Asserts the reading of a run log: a full sweep, a partial one that names its failures and says how much of the set was refused, a lock that let nothing through, and a run stopped on the clock. Also that a sentence printed at the same indent as the failing list is never read as a walk name. Run it after touching `sweep-parse.mjs` or the summary lines in `Scripts/playtest-all.sh`. |
