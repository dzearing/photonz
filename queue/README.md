...
| `bin/sweep-report.mjs` | Turns a failing sweep into ONE standing task, "Walks that fail in the full sweep", updated in place on every later sweep instead of filed again. It rewrites only the block between the two markers in `bin/sweep-notes.mjs`; anything written below the end marker is a person's triage and is carried across untouched. Until 2026-09-13 it replaced the whole notes field, and a manager pass's triage of twelve failing walks lasted four hours. |
