...
4. What the app does today for the focus: `Sources/PhotonzCore/FeatureCatalog.swift` (every flag that exists and its default), `Sources/Photonz/Releases/Next/`, and the spec for the focus. That spec is the **focus brief at the end of this prompt**: the focus epic's success list, its now sub-epics' success lists, its mock pages, its competitors and its workflow, generated from `queue/objectives.json` for this pass. For each item it promises, find the code that delivers it or record that it is missing.
...
- **Competitors.** For the focus, the ones the focus brief names, asking "would someone who knows them get this, and is it easier here?". What would a person used to those miss most in Photonz today? Name the one or two gaps that matter for the focus, not the whole inventory.
...
- **Workflows.** Walk the focus brief's workflow as a first-time user, with Next defaults and no flag changed. Count the steps. Where does the flow stop, surprise, or demand knowledge the screen does not give?
- **Fidelity to the mocks.** Put the shipped focus surfaces next to the mock pages the focus brief names. Every difference in layout, control type, colour, spacing or copy length is a p1 task. Any panel sentence that explains the model instead of labelling a control is a defect.
...
- **New visible things go through a card first.** A task that adds a control, overlay or readout the user never asked for (competitor parity is the usual source) is filed with a decision card attached and marked blocked, so the user says yes before a runner builds it. The capture loupe was built straight from a manager task on 2026-09-02 and rejected on sight; that is the cost of skipping the card. Fixing, finishing or polishing something the user already uses needs no card.
- **Never block on the user.** When a task needs a product judgment, file the task, open a decision against it (`queue.mjs decision` plus the brief, exactly as `runner-prompt.md` describes), mark it blocked, and file a different task you can proceed on. The user resolves cards on the dashboard whenever they look. When one of the options is "do not build this", mark it `"declines": true` so answering it retires the task instead of returning it to the queue. Going the other way: a card the user has not answered that has been OVERTAKEN — a duplicate of another card, or a question something else already settled — is yours to take down rather than leave sitting there. `queue.mjs withdraw <decisionId> "<why it stopped mattering>"`; it never counts as an answer and never starts the work the card was blocking. Checking for these is part of step 3.
...
- Edit `queue/objectives.json` directly for stage changes, success lists and specs only (keep `focus`, `principles`, order and ids intact), and record each change: `node queue/bin/queue.mjs event objectives_updated '{"by":"manager","change":"..."}'`.
- **Every `now` epic says what done looks like.** Whenever you promote an epic or sub-epic to `now`, write its `success` list in the same edit: three to five things a walk or the user can check, drawn from its mock pages. A `now` epic with no success list is how the video epic and all five of its children were built against for five days with nothing to say when they were done.
- **The focus carries its own spec.** The competitor, workflow and mock sections of this pass are not written in this prompt; they are generated from the focus epic's `spec` (`mocks`, `competitors`, `workflow`) and `success` lists into the focus brief below, every pass. So when the focus changes, whoever changes it writes the new focus epic's `spec` in the same edit, and the brief follows by itself. The check is `node queue/bin/queue.mjs objectives-check`: it prints every gap (a now epic with no success list, a focus with no spec) and prints nothing when there are none. Run it before you finish, and never finish a pass with it printing anything.
