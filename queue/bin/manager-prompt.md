...
4. What the app does today for the focus: `Sources/PhotonzCore/FeatureCatalog.swift` (every flag that exists and its default), `Sources/Photonz/Releases/Next/`, and the spec for the focus: the focus epic's `success` list in objectives.json first, then its mocks (for video-editing: `docs/design/mocks/pages/video*.html`, `comp-video.html`, and UX-PATTERNS D18 (a document with time has tracks); for measure-redline: `docs/design/next-measure.md`, "Done when"). For each item the spec promises, find the code that delivers it or record that it is missing.
...
- **Competitors.** For the focus. Video: Premiere Pro, Final Cut, Screen Studio and ScreenFlow, asking "would a Premiere editor get this, and is it easier here?", and Photoshop for anything drawn on the picture (text layers, moving from A to B, growing and shrinking, transitions). Redlining: CleanShot X, Shottr, Figma and Sketch. What would a person used to those miss most in Photonz today? Name the one or two gaps that matter for the focus, not the whole inventory.
...
- **Workflows.** Walk the focus's real workflow as a first-time user, with Next defaults and no flag changed (video: open a recording, cut it down, add a clip, a title and captions, transition, export; redlining: capture, measure, annotate, hand off). Count the steps. Where does the flow stop, surprise, or demand knowledge the screen does not give?
- **Fidelity to the mocks.** Put the shipped focus surfaces next to their mock frames. Every difference in layout, control type, colour, spacing or copy length is a p1 task. Any panel sentence that explains the model instead of labelling a control is a defect.
