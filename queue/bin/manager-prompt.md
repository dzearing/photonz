...
4. What the app does today for the focus: `Sources/PhotonzCore/FeatureCatalog.swift` (every flag that exists and its default), `Sources/Photonz/Releases/Next/`, and the spec for the focus: the focus epic's `success` list in objectives.json first, then its mocks (for video-editing: `docs/design/mocks/pages/video*.html`, `video-shell.html`, `comp-video.html`; for measure-redline: `docs/design/next-measure.md`, "Done when"). For each item the spec promises, find the code that delivers it or record that it is missing.
