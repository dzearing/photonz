...
- Dedupe: merge duplicate or overlapping tasks (keep the better-written one, fold notes in, drop the other with `node queue/bin/queue.mjs status <id> dropped "duplicate of <keeper>"`). `node queue/bin/queue.mjs search "<the words of the problem>"` finds near-twins that share no words in their titles, which is most of them.
- Apply the follow-up bar (at the end of this prompt) to what is already in the queue, not just to what you file. A task nobody using the app, reading the dashboard or running the loop would notice does not belong in the queue: fold its content into the task nearest it with `node queue/bin/queue.mjs log <id> "..."` and drop it, saying where it went. Anything describing something actually broken stays, however small.
...
- Spot-check the evidence on every new `p0-critical` and `p1-high` bug before it is allowed to run first. A bug the loop filed against itself must carry, in its `notes`, the command that shows the problem and that command's output (`queue/bin/runner-prompt.md` requires it). Run that command yourself. If it passes, or if there is no command in the notes at all, the bug does not get the top of the queue: drop it to `p2-normal`, retitle it as the open question it really is, and record in its log what you ran and what you saw. Two unreproduced p1 bugs on consecutive days in September 2026 each spent a runner cycle at the front of the queue on a problem that did not exist. A bug whose `source` is `user` is exempt: their report is the evidence and it stays p1.
...
- Settle any leftovers still in limbo: `node queue/bin/leftovers.mjs list` names every set of changed files a runner walked out on that nobody has taken back. A record whose task is still open clears itself the next time that task is claimed, so leave it. A record belonging to a task that is finished or dropped, or to the digest or manager pass, has nobody coming back for it: look at what is in it (`git stash show -p <sha>`), and either file a task to finish the work or drop the stash, then `node queue/bin/leftovers.mjs clear <recordId> "<what you decided>"`. Never clear one without looking: the whole point of the record is that somebody decides rather than it being committed by accident.
