...
- **Rectangle corner radius:** `cornerRadius` rounds the rectangle's *own stroke* — `AnnotationRasterizer` strokes a `CGPath(roundedRect:)` (clamped to a capsule) — rather than relying on the layer-level `LayerStyle.cornerRadius`, whose rounded mask would clip the sharp stroke corners away (the "rectangle borders disappear when rounded" bug). Edited via the ONE "Corner Radius" slider under Effects (2026-09-03): the panel used to carry two of them, one here in the shape's own section and one under Effects, both labelled Corner Radius, and they fought — pulling the Effects one chopped the rectangle's corners off. Now `CornerRadiusRow` (`AnnotationInspector.swift`) drives `PhotonzDocument.setCornerRadius`, which writes `AnnotationContent.cornerRadius` for a rectangle, `LayerStyle.cornerRadius` for everything else, and clears the old mask off a rectangle it touches. `CornerRadiusSelection` (PhotonzCore) is what the row reads. Rectangles stroke with **MITER joins** (2026-07-03): the old round joins made a thick stroke fake a corner radius the inspector read as 0 — radius 0 must be truly sharp; a real `cornerRadius` curves the path itself.
- **ONE outline, and it is a Border (2026-09-08):** the line round a shape is a single ring, and it lives in ONE place: the Effects list. A layer used to be able to draw two — a shape's own stroke and a ring its styling drew — and at the same width the two land on identical pixels, so with both set the second silently won in a different colour. The Appearance panel then made it worse by offering an Outline row beside a Border you could add, which the user reported on 2026-09-07 as two controls with no way to tell which one you were looking at. So a box and an oval have no stroke of their own at all: their edge is a `BorderEffect`, with its own colour (gradient and all), width and position, and it can be retuned, reordered or taken off with the cross like any other entry. A freshly drawn shape arrives with one already listed (`AnnotationStyles.arrivingStyle`), so nothing you draw looks different. A document saved before this opens with its stroke converted, painting the same pixels (`Layer.retiringItsOutline`, `Sources/PhotonzCore/OutlineRetirement.swift`; proved in `OutlineFoldRenderTests`). A line and an arrow are the exception: their stroke IS the layer, so it stays in the shape's own settings beside the ending and the head size. On a label a Border outlines the LETTERS rather than the box, which is what a layer ring always did there. The one thing that is not pixel-identical is a rounded box: a stroke rode a path half its width inside the frame, so an 8pt corner under a 3pt line used to read 9.5pt at the outer edge; the ring hugs the box, so Corner Radius now means the curve you can see and the number you typed is left alone.
...
- **A turned box, an upright picture (2026-09-16):** turning a lens or a magnifier turns the BOX. What shows through it does not tip over with it: a blurred region stays lined up with the picture around it, and a magnified label at a jaunty angle is still readable. The renderer reads the canvas over the area the TURNED box covers, adjusts or magnifies it there, and takes the turn back out before the shared styling path puts it back (`DocumentRenderer.canvasTurn` / `upright`). The corner, the ring, the blur and the shadow all still turn with the frame, and a lens at no turn at all is byte for byte the pixels it always was, because both helpers short-circuit on an identity transform. A turned lens also WATCHES a bigger region (`Layer.backdropSource` applies the turn), so a change in a corner the turn swung into view redraws it.
- **It is the Lens set to Magnify now (2026-09-16, Next only):** the Zoom Callout and the Lens were two tools doing the same kind of thing — both draw the picture underneath, one magnified and one changed — so with `next-lens` on there is one slot on the bar and one **Lens** section in the panel. `LensKind` is the six the Lens tool offers: the five `LensAdjustment`s, plus `magnify`. **Nothing in the model moved.** A magnifier is still a `LayerContent.zoomCallout`, so the renderer, the SVG exporter, the dirty-region logic and every saved document are untouched, and a document opened in Current still has its Zoom Callout tool and its Zoom Callout section. Migrating the content on decode was considered and rejected for exactly that reason: the Lens ships in Next only, so a Current user who picked their migrated callout would have had no Magnification and no Shape at all.
  - The bar: `ToolBarLayout.bar(withLens:)` puts `.lens` in the callout's slot rather than beside it. `.zoomCallout` remains a working `Tool` (the flat bar, the overflow menu and a playtest `tool` step can still reach it), it simply has no slot in Next.
  - The keyboard: K is the Lens as you left it, Z is the Lens set to Magnify (`EditorView.magnifyShortcut`), the same letter-owns-a-member idiom `ToolGroup` uses. Z has meant "magnify a bit of this picture" since the callout existed and its button is gone, so without this it would silently do nothing.
  - Switching a picked layer between the six is `LensConversion`. Magnify to an adjustment keeps the box exactly where it is — you are looking at that box, it must not move out from under you. An adjustment to Magnify takes the lens's box as the SOURCE (the region it was covering, which is the region you meant) and re-places the box the way a freshly drawn callout is placed, and gives the layer the callout's ring if it has none: the source outline and the leader lines are drawn entirely in the ring's colour and weight, so a ringless magnifier is a one-point black hairline.
- **Where a callout is edited (2026-09-04):** its own two settings, Magnification and Shape, are the **Zoom Callout** section in the dock (`CalloutInspector`, `InspectorSectionID.callout`) in Current, and the same two rows (`MagnifierSettingsRows`) inside the Lens section in Next. They used to live in the tool bar's style popover, which a picked callout can never open: that popover belongs to the tool in your hand, and picking up a drawing tool clears the layer selection, so neither control could be reached at all. Everything else about a callout is a layer property with one home already: its ring is the LAYER's border, so the ring's color is the Border row in the Color section and its thickness is the Border slider in Effects, exactly as for any other layer. There is no `setCalloutBorderColor` any more, and the popover has no callout half.
