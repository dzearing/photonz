...
| `reveal` | `control`, optional `in` | Scrolls whatever the control is sitting in until a press could land on it, and says how far that turned out to be. This is the step to reach for, and `scrollPanel` is for the walk that wants a particular DISTANCE scrolled. `control` and `in` are read exactly as `press` reads them. A control already in reach costs nothing and the log says so. A control that no amount of scrolling can reach — off the window itself, or in a section that is shut — fails the step and says which of the two it is. |
