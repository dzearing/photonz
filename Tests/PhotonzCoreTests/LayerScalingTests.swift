...
struct LayerScalingTests {
    ...
    /// (0, 0) and a 60×20 label at local (8, 4). Its box is (50, 60, 100, 40).
    private func card() -> Layer {
        Layer(name: "Card",
              content: .group(GroupContent(children: [
                  box("Box", CGRect(x: 0, y: 0, width: 100, height: 40)),
                  text("Label", CGRect(x: 8, y: 4, width: 60, height: 20)),
              ])),
    ...
    func childrenScaleWithTheBox() {
        ...
        #expect(child(resized, "Label")?.frame == CGRect(x: 16, y: 8, width: 120, height: 40))
    }
    ...
    func aSameSizeBoxIsAPlainMove() {
        ...
        #expect(child(moved, "Label")?.frame == CGRect(x: 8, y: 4, width: 60, height: 20))
    }
    ...
    func resizingThroughTheDocument() {
        ...
        #expect(doc.canvasFrame(of: label?.id ?? UUID())
                == CGRect(x: 66, y: 68, width: 120, height: 40))
    }
