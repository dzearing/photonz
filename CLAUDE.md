...
| Full walk sweep | Not yours to run: it is about 530 walks and about 105 minutes, eleven times the 600s ceiling on an agent's background work. `queue/bin/sweep.sh request "<why>"` and the go loop runs it between tasks. `Scripts/playtest-all.sh --no-build <name-fragment>` runs just the walks you touched, in seconds |
