...
extension EditorState {
    ...
    ///
    /// For a shape, edge and all: the whole of that rule lives in
    /// `AnnotationStyles.remember(_:forShape:)`, the mirror of
    /// `arrivingStyle(forShape:)` and tested beside it. An edge left on is
    /// lifted onto the tool so the next box wears one ring rather than two; an
    /// edge switched off stays a row that is off, holding its numbers; an edge
    /// taken off with the cross is gone for good.
    private func captureStyleDefault(layerID: UUID) {
        ...
        if let shape = layer.annotation?.shape {
            annotationStyles.remember(layer.style, forShape: shape)
