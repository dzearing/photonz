        ...
        [
            ...
            Definition(
                flag: FeatureFlag(
                    ...
                    description: "Right click the picture in the layers list, or use Layer then Separate into Layers, and every run of text and every box in the screenshot becomes its own layer you can pick up and move. Where a piece came from, the picture is filled in with what was around it, so dragging a label off a dark button leaves the button looking untouched rather than punching a hole in it. A box that is really one flat colour comes out as a real rounded rectangle you can resize and repaint. You get two things and not three: the pieces on their own layers and the picture with the gaps filled. Anything the app cannot read confidently is left in the picture and not mentioned. It is one undo step however many layers come out. Off means the command is absent from both menus.",
                    isEnabled: false,
                    parameters: []),
                releases: [.next],
                enabledByDefaultIn: [.next]),
