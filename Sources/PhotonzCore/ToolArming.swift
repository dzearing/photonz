...
public extension AnnotationStyles {
    ...
    ///
    /// A box's or an oval's EDGE arms the tool's outline colour, because that
    /// is what it is: their edge is a Border in the Effects list now, so the
    /// two are one choice (`OutlineRetirement.swift`). A ring round anything
    /// else, a shadow, a glow and a text block's ink go past without doing
    /// anything: those are laid over a layer after it exists rather than being
    /// what the next shape comes out wearing, and new text takes the
    /// foreground colour.
    ///
    ...
    mutating func arm(_ paint: Paint?, styleID: UUID? = nil, name: String? = nil,
        ...
        // A box's edge and the tool's outline colour are the same answer.
        let slot = (slot == .border && shape.arrivesWithABorder) ? .stroke : slot
        switch slot {
        case .stroke:
        ...
        // None of these is a colour a tool draws with: a letter takes the
